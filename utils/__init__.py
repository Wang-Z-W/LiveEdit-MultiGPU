from editor.vllm_editors.base import VLLMBaseEditorWithTraining, VLLMBaseEditor
...
def load_vllm_editor(editor_name:str, edit_model_name:str, device:List[str], 
        extra_devices:List[str] = ['cuda:0'], editor_ckpt_path = None, for_train = False):
    ...
    if editor_ckpt_path != None:
        if isinstance(editor, VLLMBaseEditorWithTraining):
            editor.load_ckpt(editor_ckpt_path, restrict = True, load_opt = False)
        else:
            editor.load_ckpt(editor_ckpt_path, device, True)
    return editor
