...
class FTvl(VLLMBaseEditor):
    ...
    def save_ckpt_eval(self, eval_cfg, save_path:str):
        ...
        editor_state = {
            ...
            'editor_config': self.cfg,
            'eval_config': eval_cfg
        }
        torch.save(editor_state, save_path)
        
    def load_ckpt(self, load_path:str, device, restrict=True):
        print(f"FTvl editor is loading checkpoint from {load_path}")
        if not os.path.exists(load_path):
            raise ValueError(f'load_path {load_path} does not exist')
        state_dict = torch.load(load_path, map_location='cpu', weights_only=False)
        self.vllm.model.load_state_dict(state_dict['vllm_model_state_dict'], strict = restrict)
        self.vllm.model.to(device)
