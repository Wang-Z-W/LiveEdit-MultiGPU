...
class LiveEdit(VLLMBaseEditorWithTraining):
    ...
    def __init__(self, vllm: BaseVLLMForEdit, config:LiveEditConfig, device:List[str] = None, 
            vllm_data_proc: BaseVLLMForEdit = None, data_proc_device:List[str] = None, verbose = False):
        ...
        self.edit_extractor = QVExtractor(eqe_n, llm_mid_dim, module_dim, cross_att_head_n, self.vllm.get_img_token_n(), False).to(self.device[-1])
        self.inpt_extractor = QVExtractor(eqe_n, llm_mid_dim, module_dim, cross_att_head_n, self.vllm.get_img_token_n(), True).to(self.device[-1])
        self.moegen_c = LowRankGenerator(llm_mid_dim, lora_rank, lora_scale, llm_mid_dim, module_dim, cross_att_head_n).to(self.device[-1])
        self.moegen_r = LowRankGenerator(llm_mid_dim, lora_rank, lora_scale, llm_mid_dim, module_dim, cross_att_head_n).to(self.device[-1])
        self.instant_reps_norm = nn.LayerNorm(self.cfg.llm_mid_dim).to(self.device[-1])
    ...
    def wrap_and_hook(self):
        ...
        def edit_with_moes(module, args, output):
            if self.is_train:
                output = apply_edit_residual(output, self.train_edit_residual.to(output[0].device))
    ...
    def restore_to_original_model(self):
        self.requests_pool = []
        self.eqr_pool = torch.zeros([0, self.cfg.retrieval_editor.eqe_n, self.cfg.retrieval_editor.module_dim], device=self.device[-1]) # editing query representation pool
        self.evr_pool = torch.zeros([0, self.cfg.retrieval_editor.eqe_n, self.cfg.retrieval_editor.module_dim], device=self.device[-1]) # editing-relevant vision representation pool
    ...
    def organize_batch_data(self, a_batch_raw_data:List):
        ...
        # Fuse moe masks
        edit_ns = torch.tensor([len(bes) for bes in batch_edit_signal], device=self.device[-1])
        cols = torch.sum(edit_ns)
        start_indices = torch.cumsum(torch.cat([torch.tensor([0], device=self.device[-1]), edit_ns[:-1]]), dim=0).unsqueeze(1)
        rel_edit_i = start_indices[:, 0] + torch.tensor(rel_edit_i, device=self.device[-1])
        rel_moe_mask = torch.zeros((batch_size, cols), dtype=torch.int, device=self.device[-1])
        rel_moe_mask[torch.arange(batch_size), rel_edit_i] = 1
        idx = torch.arange(cols, device = self.device[-1]).expand(batch_size, cols)
        ...
        return move_to_device(a_batch_organized_data, self.device[-1])
    ...
    def train_a_batch(self, a_batch_organized_data):
        ...
        loss += moe_soft_routing_loss.to(loss.device) * self.cfg.train_cfg.soft_routing_lambda
        # Hard Routing losses
        ...
        loss += hard_routing_loss.to(loss.device) * self.cfg.train_cfg.hard_routing_lambda
        # update
        loss.backward()
        self.opt.step()
        self.opt.zero_grad()
    ...
    def get_a_new_optimizer(self)->torch.optim.Optimizer:
        ...
        lr_cut_it = torch.tensor(self.cfg.train_cfg.lr_cut_it, device=self.device[-1])
        def lr_lambda(step):
