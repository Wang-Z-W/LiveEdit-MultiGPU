...
def get_attr():
    ...
    parser.add_argument('-dfn', '--data_filename', type=str, default = None, help = 'Filename for evaluation. Mainly for SpuMNIST dataset.')
...
if __name__ == '__main__':
    ...
    if cfg.data_name == 'EVQA':
        ...
        data_path = os.path.join(ROOT_PATH, f'data/WaterBird/{cfg.data_filename}.json')
        img_root_dir = os.path.join(ROOT_PATH, 'data/WaterBird')
        eval_data = WaterBird(data_path, img_root_dir, cfg.split, cfg.data_sample_n)
        cfg.data_sample_n = len(eval_data.data_with_img)
    else:
        eval_data = None
    # evaluate
